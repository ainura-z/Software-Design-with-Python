...
from Task4 import decorator_3_exception


@decorator_1
def sort_list_of_tuples(list):
    """
    This function sorts a list of tuples.
    """
    print(f"Original list of tuples: {list}.")
    list.sort(key=lambda x: x[1])
    print(f"Sorted List of Tuples: {list}.")
...
def find_palindromes(list):
    """
    This function looks for palindromes in a given list of strings.
    """
    result = list(filter(lambda s: s == s[::-1], list))
    print(f"List of palindromes: {result}.")
...
def pascal_triangle(n):
    ...
    if n < 0:
        ...
        raise ValueError("n argument cannot be less than 0.")
...
def quadratic_equation_solver(a, b, c=1):
    """
    This function solves a quadratic equation a*x^2 + b*x + c = 0
    and prints the result.
    """
    if a == 0:
        print("This is a linear equation.")
        raise ValueError("a argument cannot be 0.")
    ...
    if discriminant > 0:
        ...
        print("D < 0:\nTwo complex roots:")
        print(
            f"x1 = {x1_real} + i*{x1_imaginary:.4f} \nx2 = {x1_real} - i*{x1_imaginary:.4f}"
        )
...
if __name__ == "__main__":
    ...
    find_palindromes(palindrome_list)
